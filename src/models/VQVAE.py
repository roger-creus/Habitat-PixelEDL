...
class VectorQuantizer(nn.Module):
    def __init__(self, num_embeddings, embedding_dim, commitment_cost):
        ...
        self._embedding.weight.data.uniform_(-1/self._num_embseddings, 1/self._num_embeddings)
...
class VectorQuantizerEMA(nn.Module):
    ...
    def compute_distances(self, inputs):
        ...
        #flat_input_linear = nn.Linear(64*8*8, self._embedding_dim)
        #flat_input = flat_input_linear(flat_input)
        distances = (torch.sum(flat_input**2, dim=1, keepdim=True)
                    + torch.sum(self._embedding.weight**2, dim=1)
                    - 2 * torch.matmul(flat_input, self._embedding.weight.t()))
        return distances
...
class Encoder(nn.Module):
    def __init__(self, in_channels, num_hiddens, num_residual_layers, num_residual_hiddens):
        ...
        self._conv_6 = nn.Conv2d(in_channels=num_hiddens,
                                 out_channels=num_hiddens,
                                 kernel_size=4,
                                 stride=2, padding=1)
        self._conv_7 = nn.Conv2d(in_channels=num_hiddens,
                                 out_channels=num_hiddens,
                                 kernel_size=4,
                                 stride=2, padding=1)
...
class Decoder(nn.Module):
    def __init__(self, in_channels, num_hiddens, num_residual_layers, num_residual_hiddens):
        ...
        self._conv_trans_7 = nn.ConvTranspose2d(in_channels=num_hiddens//4,
                                                out_channels=3,
                                                kernel_size=4,
                                                stride=2, padding=1)
...
class VQVAE_PL(pl.LightningModule):
    ...
    def compute_reward(self, z_a):
        ...
        min = torch.min(distances).detach().cpu().item()
        return 1 / (1 + min)
