...
class PixelVQVAE(pl.LightningModule):
    def __init__(self, num_hiddens=64, num_residual_layers=2, num_residual_hiddens=32,
        ...
        self.img_mlp = nn.Sequential(
            nn.Linear(self.n_h * self.k * self.k, int(self.n_h * self.k * self.k/2)),
            nn.ReLU(),
            nn.Linear(int(self.n_h * self.k * self.k/2), int(embedding_dim))
        )
        ...
        self.img_mlp_inv = nn.Sequential(
            nn.Linear(int(embedding_dim),int(self.n_h * self.k * self.k/2)),
            nn.ReLU(),
            nn.Linear(int(self.n_h * self.k * self.k/2),self.n_h * self.k * self.k)
