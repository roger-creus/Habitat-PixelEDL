...
from src.config_RL import setSeed, getConfig
from collections import OrderedDict, defaultdict, Counter
from src.main.utils_RL import *
...
from src.customLoader import *
from torchvision.transforms import transforms

from src.models.CURL_RL import CURL_PL
...
class CURL(CURL_PL):
    ...
    def forward(self, data):
        key, query = data[:,0].permute(0,3,1,2), data[:,1].permute(0,3,1,2)
    ...
    def forward_single(self, image):
        ...
        #return np.array(self.encode(torch.FloatTensor(image.float().cpu())).detach().cpu().numpy()).squeeze()
        return np.array(self.encode(torch.FloatTensor(image.float().cpu()).cuda()).detach().cpu().numpy()).squeeze()
    ...
    def train_dataloader(self):
        train_dataset = CustomHabitatData(self.trajectories_train, transform=self.transform, delay=self.delay, **self.conf)
        train_dataloader = DataLoader(train_dataset, batch_size=self.batch_size, shuffle=True, num_workers=10)
        return train_dataloader

    def val_dataloader(self):
        val_dataset = CustomHabitatData(self.trajectories_val, transform=self.transform, delay=self.delay, **self.conf)
    ...
    def _store_goal_positions(self):
        store_goal_positions(self)

    def _construct_map(self):
        construct_map(self,"curl")
