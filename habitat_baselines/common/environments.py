...
class SkillNavEnv(habitat.RLEnv):
    ...
    def get_reward(self, observations):
        ...
        #if self._episode_success():
        #   reward += self._rl_config.SUCCESS_REWARD
        rgb_obs = torch.FloatTensor(observations["rgb"]).unsqueeze(0).cuda()
        perception_embed = torch.FloatTensor(self.enc.forward_single(rgb_obs)).cuda()
        
        closest = self.enc.compute_argmax(perception_embed)
        
        if closest == int(self._env.current_episode.episode_id):
            return 1
        else:
            return 0
